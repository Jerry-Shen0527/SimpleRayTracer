...
public:
	...
	static RGBSpectrum FromRGB(const Float rgb[], SpectrumType type = SpectrumType::Illuminant);
	static RGBSpectrum FromXYZ(const Float xyz[], SpectrumType type = SpectrumType::Illuminant);
...
inline void RGBSpectrum::ToRGB(Float rgb[3]) const
{
	rgb[0] = c[0];
	rgb[1] = c[1];
	rgb[2] = c[2];
...
inline void RGBSpectrum::ToXYZ(Float xyz[3]) const
{
	Float rgb[3];
	ToRGB(rgb);
	RGBToXYZ(rgb, xyz);
